...
At the beginning, the R script must read the status file. This should be done with the function `Init`.
...
For writing a log or sending a status update (like 40\% or so) to the shiny session functions `Log` and `Up` can be used.
At the very end of the R script, the `Fin` function should be used which signals the shiny session that the R script has finished.
All the outputs are written and the R session is quit.
There is also a function -- `Try` -- which can be used for stuff that might produce an error.
If the R script runs into an error and quits the shiny session will never know about it.
`Try` just makes sure that the shiny session is informed about that with some kind of error message.
...
The functions `Init`, `Log`, `Up`, `Try`, `Fin` all check whether they are still the *newest* process before they write anything.

#### Directories and Logs

If you read the documentation for `RProcessStart` you will see that there can be several directories.
...
The R script will set its working directory to `pwd` with the call to `Init`.
...
Otherwise use the `Log` function to write something in there.
...
Then, start the following app from the current directory.
...
Process 1 will start process 2 using the `Start` function.
...
Sys.sleep(4)
res <- shinyTools::Try(dpois(1:100, input))
shinyTools::Start("process2", "/absolute/path/to/scripts/process2.R",
...
But before finishing with the `Fin` function process 1 starts another R script using id `process2`.
